...
def visualize():
    ...
    for idx in range(min(CONFIG["num_samples"], len(dataset))):
        ...
        if CONFIG["show_augmentations"] and dataset_aug is not None:
            ...
            image_aug = (image_aug * 255).clamp(0, 255).permute(1, 2, 0).numpy().astype(np.uint8)
            ...
            if len(mask_labels_aug) > 0:
                ...
                annotated_image_aug = mask_annotator.annotate(scene=image_aug.copy(), detections=detections_aug)
            else:
            ...
            cv2.putText(annotated_image, "Original", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            cv2.putText(annotated_image_aug, "Augmented", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            annotated_image = np.concatenate([annotated_image, annotated_image_aug], axis=1)
